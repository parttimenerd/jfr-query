...
public class QueryCommand implements Callable<Integer> {
            ...
            try {
                if (!Files.exists(path) || Files.getLastModifiedTime(path).toMillis() <
                    ...
                    try {
                        ...
                        System.err.println("Error transforming JFR file into a database: " + inputFile + " (No such file or directory)");
                        return 1;
                    } catch (IOException e) {
                        if (isInvalidJfr(e)) {
    ...
    private static boolean isInvalidJfr(Throwable e) {
        for (Throwable t = e; t != null; t = t.getCause()) {
            String msg = t.getMessage();
            if (msg == null) continue;
            if (msg.equals("Not a complete Chunk header")
                    || msg.equals("Chunk contains no data")
                    || msg.contains("Invalid JFR Magic Number")) {
                return true;
            }
        }
        return false;
