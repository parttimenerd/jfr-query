/**
 * QA Session S167 — GC Deep Dive + ZGC Analysis
...
(async () => {
  ...
  // ── SECTION 9: Template: GC Deep Dive ───────────────────────────────────
  console.log('\n── Template: GC Deep Dive ──');
  try {
    const gcPage = await loadTemplate(browser, 'GC Deep Dive');
    const gcCells = await gcPage.locator('.prose, [data-cell]').count();
    const gcCharts = await gcPage.locator('.recharts-wrapper').count();
    const gcErrors = await domScan(gcPage);
    check('GC Deep Dive: cells rendered', gcCells > 0, `cells=${gcCells}`);
    check('GC Deep Dive: DOM clean', gcErrors.length === 0, gcErrors.join('; '));
    console.log(`  charts=${gcCharts}`);
    if (gcCharts > 0) {
      const zoomed = await tryLinkXZoom(gcPage);
      check('GC Deep Dive: LINK_X zoom works', zoomed);
    }
    await gcPage.close();
  } catch (e) {
    fail++; failures.push('GC Deep Dive: ' + e.message.slice(0, 120));
    console.log('  ❌ GC Deep Dive FAILED: ' + e.message.slice(0, 120));
  }

  // ── SECTION 10: Template: ZGC Analysis ──────────────────────────────────
  console.log('\n── Template: ZGC Analysis ──');
  try {
    const zgcPage = await loadTemplate(browser, 'ZGC Analysis');
    const zgcCells = await zgcPage.locator('.prose, [data-cell]').count();
    const zgcCharts = await zgcPage.locator('.recharts-wrapper').count();
    const zgcErrors = await domScan(zgcPage);
    check('ZGC Analysis: cells rendered', zgcCells > 0, `cells=${zgcCells}`);
    check('ZGC Analysis: DOM clean', zgcErrors.length === 0, zgcErrors.join('; '));
    console.log(`  charts=${zgcCharts}`);
    await zgcPage.close();
  } catch (e) {
    fail++; failures.push('ZGC Analysis: ' + e.message.slice(0, 120));
    console.log('  ❌ ZGC Analysis FAILED: ' + e.message.slice(0, 120));
  }
  ...
  // ── Summary ───────────────────────────────────────────────────────────────
  console.log('\n══════════════════════════════════════════');
  console.log('S167 QA: ' + pass + ' passed, ' + fail + ' failed');
