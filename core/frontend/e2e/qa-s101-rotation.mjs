/**
 * S101 template rotation: Heap Allocation + I/O & Latency
 */
...
async function domScan(page) {
  return page.evaluate((t)=>Array.from(document.querySelectorAll('*')).filter(el=>{
    ...
    return t.some(x=>tx.includes(x))&&el.children.length===0&&el.offsetParent!==null&&!el.closest('.cm-editor')&&!el.closest('[class*="token"]');
...
(async()=>{
  ...
  const demo=await page.$('button:has-text("Try the demo")'); if(!demo){console.error('no demo');process.exit(1);}
  ...
  for (const name of ['Heap Allocation','I/O & Latency']) {
    ...
    if(e.length===0)pass(`${name}: ${cells} cells, ${svgs} SVGs, 0 DOM errors`);
    else fail(name,e.join(' | '));
  }
