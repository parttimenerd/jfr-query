/**
 * S124 QA: demo + full interactive suite (variables, LINK_X, BRUSH probe,
 * command palette, autocomplete, schema explorer, Run All, help modal,
 * tooltips, UI polish) + I/O & Latency + Container & Cloud
 */
...
// Dispatch a WheelEvent on the first large div.group plot wrapper
async function dispatchZoom(page){
...
(async()=>{
  ...
  // Help modal
  {
    ...
    if(hBtn){
      ...
      if(dlg){const ok=await dlg.$('table,kbd,[class*="shortcut"],code');if(ok)pass('Help modal: shortcut content visible');else warn('Help modal: no shortcut content');}
  ...
  {
    const zoomed=await dispatchZoom(page);
    await page.waitForTimeout(800);
    if(zoomed){
  ...
  // BRUSH test — look for a bar chart cell with BRUSH clause, hover it
  console.log('\n══ BRUSH / TOOLTIP ══');
  {
    // Tooltip: hover first available chart
    ...
    for(const ch of charts.slice(0,5)){
      ...
      const tip=await page.$('.recharts-tooltip-wrapper,[data-testid="plot-tooltip"],[class*="PlotTooltip"],[class*="tooltip-content"]');
      if(tip){tooltipFound=true;pass('Tooltip: appeared on chart hover');break;}
    }
    if(!tooltipFound)warn('Tooltip: not found on any chart (may need data)');

    // BRUSH: look for a cell that has BRUSH in its plot code
    const brushFound=await page.evaluate(()=>{
      const editors=Array.from(document.querySelectorAll('.cm-editor'));
      return editors.some(e=>(e.textContent||'').includes('BRUSH'));
    });
    if(brushFound)pass('BRUSH: cell with BRUSH clause found in notebook');
    else warn('BRUSH: no BRUSH cell found in GC Pause Analysis');
  }

  // UI polish checks
  console.log('\n══ UI POLISH ══');
  {
    const zeroH=await page.evaluate(()=>Array.from(document.querySelectorAll('[data-cell-status]')).filter(el=>el.offsetParent!==null&&el.getBoundingClientRect().height<5).length);
    if(zeroH===0)pass('UI: no zero-height cells');else warn(`UI: ${zeroH} zero-height cells`);
    const overflowX=await page.evaluate(()=>{const el=document.querySelector('main,[role="main"],.notebook-content')||document.body;return el.scrollWidth>el.clientWidth+5;});
  ...
  console.log('\n══ I/O & LATENCY ══');
  {
    const ok=await loadTpl(page,'I/O & Latency');
    if(ok){
      ...
      e=await scan(page);
      if(e.length===0)pass(`I/O & Latency: ${c} cells, ${s} SVGs, 0 errors`);else fail('I/O & Latency',e.join(' | '));
      await scroll(page);e=await scan(page);
      if(e.length===0)pass('I/O & Latency: scroll 0 errors');else fail('I/O & Latency scroll',e.join(' | '));
    } else fail('I/O & Latency','did not idle');
  }
