/**
 * S125 QA: demo + full interactive suite + Threading & Contention + Exceptions & Errors
 */
...
(async()=>{
  ...
  console.log('\n══ EXCEPTIONS & ERRORS ══');
  {
    ...
    if(ok){
      ...
      const s=await page.evaluate(()=>document.querySelectorAll('svg').length);
      e=await scan(page);
      if(e.length===0)pass(`Exceptions & Errors: ${c} cells, ${s} SVGs, 0 errors`);else fail('Exceptions & Errors',e.join(' | '));
      await scroll(page);e=await scan(page);
      if(e.length===0)pass('Exceptions & Errors: scroll 0 errors');else fail('Exceptions & Errors scroll',e.join(' | '));
