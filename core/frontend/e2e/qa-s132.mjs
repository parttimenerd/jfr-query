/**
 * S132 QA: demo + full interactive suite (incl. BRUSH, tooltips, resize)
 *         + Container & Cloud + Threading & Contention
 */
...
(async()=>{
  ...
  const svgs0=await page.evaluate(()=>document.querySelectorAll('svg').length);
  const cells0=await page.evaluate(()=>document.querySelectorAll('[data-cell-status]').length);
  pass(`Demo: ${cells0} cells, ${svgs0} SVGs`);
  ...
  // Schema explorer — expand table, check columns with types
  {
    ...
    if(gcBtn){
      ...
      if(prev){
        const txt=await prev.evaluate(el=>el.textContent||'');
        const hasTypes=txt.includes('INTEGER')||txt.includes('TIMESTAMP')||txt.includes('VARCHAR')||txt.includes('DOUBLE');
        if(hasTypes)pass('Schema explorer: preview with types appeared');
        else pass('Schema explorer: preview appeared (no type info visible)');
  ...
  await page.keyboard.press('Meta+k');await page.waitForTimeout(500);
  {
    ...
    else{warn('Command palette: dialog not found');await page.keyboard.press('Escape').catch(()=>{});}
  }
  ...
  // SQL autocomplete
  {
    ...
    if(ed){
      ...
      if(added){const del=await page.$$('[aria-label="Delete Cell"],[title="Delete Cell"],[aria-label="Delete cell"]');if(del.length>0)await del[del.length-1].click().catch(()=>{});}
    } else warn('SQL autocomplete: no editor found');
  }

  // Help modal
  {
    const hBtn=await page.$('[aria-label="Keyboard Shortcuts"],[title*="Keyboard Shortcuts"],[aria-label*="Help"],[title*="Help"]');
    if(hBtn){
      ...
      if(dlg){
        ...
        const txt=await dlg.evaluate(el=>el.textContent||'');
        if(ok||txt.length>100)pass('Help modal: content visible');
        else warn('Help modal: opened but minimal content');
      } else warn('Help modal: no dialog');
      await page.keyboard.press('Escape');await page.waitForTimeout(200);
    } else warn('Help modal: button not found');
  }

  // LINK_X zoom — load GC Pause Analysis first
  console.log('\n══ LINK_X ZOOM ══');
  await loadTpl(page,'GC Pause Analysis');
  {
    const zoomed=await page.evaluate(()=>{
      const g=Array.from(document.querySelectorAll('div.group')).filter(g=>g.querySelector('.recharts-surface')&&g.querySelector('.recharts-wrapper')).find(g=>g.getBoundingClientRect().width>300);
      if(!g)return false;
      g.scrollIntoView({behavior:'instant',block:'center'});
      const r=g.getBoundingClientRect();
      g.dispatchEvent(new WheelEvent('wheel',{bubbles:true,cancelable:true,deltaY:-300,deltaMode:0,clientX:r.x+r.width/2,clientY:r.y+r.height/2,shiftKey:true}));
      return true;
    });
    ...
    if(zoomed){
      ...
      else warn('LINK_X zoom: Reset button not found after wheel event');
    } else warn('LINK_X zoom: no plot group found');
    e=await scan(page);if(e.length===0)pass('GC Pause Analysis: 0 DOM errors');else fail('GC Pause Analysis',e.join(' | '));
  }
  ...
  // Chart tooltip hover
  {
    const moved=await page.evaluate(()=>{
      ...
      surface.scrollIntoView({behavior:'instant',block:'center'});
      const r=surface.getBoundingClientRect();
      surface.dispatchEvent(new MouseEvent('mousemove',{bubbles:true,clientX:r.x+r.width/2,clientY:r.y+r.height/2}));
      return true;
    });
    await page.waitForTimeout(600);
    if(moved){
      const tooltip=await page.$('.recharts-tooltip-wrapper,[class*="tooltip"]');
      if(tooltip)pass('Chart tooltip: tooltip element present on hover');
      else warn('Chart tooltip: no tooltip visible after mousemove');
    } else warn('Chart tooltip: no recharts surface found');
  }

  // BRUSH — load Comprehensive Feature Test which has BRUSH cells
  console.log('\n══ BRUSH ══');
  {
    const ok=await loadTpl(page,'Comprehensive Feature Test');
    if(ok){
      // find a BRUSH chart — look for brush overlay element
      const brushFound=await page.evaluate(()=>{
        const brushEls=document.querySelectorAll('.recharts-brush,.recharts-brush-traveller,[class*="brush"]');
        return brushEls.length>0;
      });
      if(brushFound){
        pass('BRUSH: brush element found in Comprehensive Feature Test');
        // try dragging to trigger selection
        const dragged=await page.evaluate(()=>{
          const brush=document.querySelector('.recharts-brush-slide');
          if(!brush)return false;
          brush.scrollIntoView({behavior:'instant',block:'center'});
          const r=brush.getBoundingClientRect();
          brush.dispatchEvent(new MouseEvent('mousedown',{bubbles:true,clientX:r.x+10,clientY:r.y+r.height/2}));
          document.dispatchEvent(new MouseEvent('mousemove',{bubbles:true,clientX:r.x+80,clientY:r.y+r.height/2}));
          document.dispatchEvent(new MouseEvent('mouseup',{bubbles:true,clientX:r.x+80,clientY:r.y+r.height/2}));
          return true;
        });
        await page.waitForTimeout(500);
        if(dragged)pass('BRUSH: drag dispatched');
        else warn('BRUSH: could not find brush slide element');
      } else warn('BRUSH: no brush elements found in Comprehensive Feature Test');
      e=await scan(page);
      if(e.length===0)pass('Comprehensive Feature Test: 0 DOM errors');else fail('Comprehensive Feature Test',e.join(' | '));
    } else fail('Comprehensive Feature Test','did not idle');
  }

  // ── CONTAINER & CLOUD ──
  console.log('\n══ CONTAINER & CLOUD ══');
  {
    const ok=await loadTpl(page,'Container & Cloud');
    if(ok){
  ...
  console.log('\n══ THREADING & CONTENTION ══');
  {
    const ok=await loadTpl(page,'Threading & Contention');
    if(ok){
      const c=await page.evaluate(()=>document.querySelectorAll('[data-cell-status]').length);
      const s=await page.evaluate(()=>document.querySelectorAll('svg').length);
      e=await scan(page);
      if(e.length===0)pass(`Threading & Contention: ${c} cells, ${s} SVGs, 0 errors`);else fail('Threading & Contention',e.join(' | '));
      await scroll(page);e=await scan(page);
      if(e.length===0)pass('Threading & Contention: scroll 0 errors');else fail('Threading & Contention scroll',e.join(' | '));
    } else fail('Threading & Contention','did not idle');
  }

  // ── UI POLISH ──
  console.log('\n══ UI POLISH ══');
  // Check for zero-height or overflow cells
  {
    const issues=await page.evaluate(()=>{
      const problems=[];
      document.querySelectorAll('[data-cell-status]').forEach(el=>{
        const r=el.getBoundingClientRect();
        if(r.height===0&&el.offsetParent!==null)problems.push(`zero-height cell: ${el.getAttribute('data-cell-status')}`);
      });
      // Check for overflow on plot wrappers
      document.querySelectorAll('[class*="plot"],[class*="chart"]').forEach(el=>{
        const st=window.getComputedStyle(el);
        if(st.overflow==='visible'&&el.scrollWidth>el.clientWidth+5)
          problems.push(`overflow: ${el.className?.toString().slice(0,60)}`);
      });
      return problems.slice(0,5);
    });
    if(issues.length===0)pass('UI polish: no zero-height cells or overflow issues');
    else warn('UI polish issues',issues.join(' | '));
  }

  // ── CONSOLE ──
  console.log('\n══ CONSOLE ══');
  if(cerrs.length===0)pass('Console: 0 real errors');
  else{cerrs.forEach(c=>console.log(`  [console error] ${c}`));fail('Console errors',cerrs.length+' errors: '+cerrs[0]);}

  await ctx.close();await browser.close();

  console.log(`\nPASS:${P.length} WARN:${W.length} FAIL:${F.length}`);
  if(F.length===0){console.log('No failures ✅');W.forEach(w=>console.log(`  ⚠  ${w}`));}
