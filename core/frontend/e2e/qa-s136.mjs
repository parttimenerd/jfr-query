/**
 * S136 QA: full interactive suite (BRUSH, tooltips, resize, UI polish)
 *         + ZGC Analysis + CPU Profiling
 */
...
(async()=>{
  ...
  // Schema explorer — check columns with types
  {
  ...
  // Chart tooltip hover
  {
    ...
    await page.waitForTimeout(600);
    if(moved){
      const tooltip=await page.$('.recharts-tooltip-wrapper,[class*="tooltip"]:not([class*="cm"])');
      if(tooltip)pass('Chart tooltip: tooltip wrapper present on hover');
      else warn('Chart tooltip: no tooltip visible after mousemove');
    } else warn('Chart tooltip: no recharts surface found');
  }

  // BRUSH — Comprehensive Feature Test has BRUSH cells
  console.log('\n══ BRUSH ══');
  {
    const ok=await loadTpl(page,'Comprehensive Feature Test');
    if(ok){
      const brushFound=await page.evaluate(()=>
        document.querySelectorAll('.recharts-brush,.recharts-brush-traveller').length>0
      );
      if(brushFound){
        pass('BRUSH: recharts-brush element found');
        const dragged=await page.evaluate(()=>{
          const slide=document.querySelector('.recharts-brush-slide');
          if(!slide)return 'no-slide';
          slide.scrollIntoView({behavior:'instant',block:'center'});
          const r=slide.getBoundingClientRect();
          slide.dispatchEvent(new MouseEvent('mousedown',{bubbles:true,clientX:r.x+10,clientY:r.y+r.height/2}));
        ...
        if(dragged==='ok')pass('BRUSH: drag interaction dispatched');
        else warn('BRUSH: brush-slide not found for drag');
      } else warn('BRUSH: no recharts-brush elements in Comprehensive Feature Test');
      e=await scan(page);
      if(e.length===0)pass('Comprehensive Feature Test: 0 DOM errors');
      else fail('Comprehensive Feature Test',e.join(' | '));
  ...
  console.log('\n══ UI POLISH ══');
  {
    const issues=await page.evaluate(()=>{
      const problems=[];
      // Zero-height visible cells
      document.querySelectorAll('[data-cell-status]').forEach(el=>{
        if(el.getBoundingClientRect().height===0&&el.offsetParent!==null)
          problems.push(`zero-height cell`);
      });
      // Overflow on plot containers
      document.querySelectorAll('.recharts-wrapper').forEach(el=>{
        if(el.scrollWidth>el.clientWidth+8)
          problems.push(`recharts-wrapper overflow: scrollWidth=${el.scrollWidth} clientWidth=${el.clientWidth}`);
      });
      return problems.slice(0,5);
    });
    if(issues.length===0)pass('UI polish: no layout issues found');
  ...
  console.log('\n══ ZGC ANALYSIS ══');
  {
    const ok=await loadTpl(page,'ZGC Analysis');
    if(ok){
      ...
      e=await scan(page);
      if(e.length===0)pass(`ZGC Analysis: ${c} cells, ${s} SVGs, 0 errors`);
      else fail('ZGC Analysis',e.join(' | '));
      await scroll(page);e=await scan(page);
      if(e.length===0)pass('ZGC Analysis: scroll 0 errors');
      else fail('ZGC Analysis scroll',e.join(' | '));
    } else fail('ZGC Analysis','did not idle');
  }
  ...
  if(cerrs.length===0)pass('Console: 0 real errors');
    ...
    fail('Console errors',`${cerrs.length} errors: ${cerrs[0]}`);
  }
