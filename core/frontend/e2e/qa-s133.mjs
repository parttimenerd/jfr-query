/**
 * S133 QA: demo + full interactive suite + Memory Leak Detection + Exceptions & Errors
 */
...
(async()=>{
  ...
  console.log('\n══ MEMORY LEAK DETECTION ══');
  {
    ...
    if(ok){
      ...
      e=await scan(page);
      if(e.length===0)pass(`Memory Leak Detection: ${c} cells, ${s} SVGs, 0 errors`);else fail('Memory Leak Detection',e.join(' | '));
      await scroll(page);e=await scan(page);
      if(e.length===0)pass('Memory Leak Detection: scroll 0 errors');else fail('Memory Leak Detection scroll',e.join(' | '));
