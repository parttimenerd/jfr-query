/**
 * S137 QA: demo + full interactive suite + Heap Allocation + I/O & Latency
 */
...
(async()=>{
  ...
  console.log('\n══ I/O & LATENCY ══');
  {
    ...
    if(ok){
      ...
      const s=await page.evaluate(()=>document.querySelectorAll('svg').length);
      e=await scan(page);
      if(e.length===0)pass(`I/O & Latency: ${c} cells, ${s} SVGs, 0 errors`);
      else fail('I/O & Latency',e.join(' | '));
      await scroll(page);e=await scan(page);
      if(e.length===0)pass('I/O & Latency: scroll 0 errors');
      else fail('I/O & Latency scroll',e.join(' | '));
