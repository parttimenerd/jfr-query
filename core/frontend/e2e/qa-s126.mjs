/**
 * S126 QA: demo + full interactive suite + Comprehensive Feature Test + ZGC Analysis
 */
...
(async()=>{
  ...
  console.log('\n══ COMPREHENSIVE FEATURE TEST ══');
  {
    ...
    if(ok){
      ...
      e=await scan(page);
      if(e.length===0)pass(`Comprehensive Feature Test: ${c} cells, ${s} SVGs, 0 errors`);else fail('Comprehensive Feature Test',e.join(' | '));
      await scroll(page);e=await scan(page);
      if(e.length===0)pass('Comprehensive Feature Test: scroll 0 errors');else fail('Comprehensive Feature Test scroll',e.join(' | '));
  ...
  console.log('\n══ ZGC ANALYSIS ══');
  {
    ...
    if(ok){
      ...
      const s=await page.evaluate(()=>document.querySelectorAll('svg').length);
      e=await scan(page);
      if(e.length===0)pass(`ZGC Analysis: ${c} cells, ${s} SVGs, 0 errors`);else fail('ZGC Analysis',e.join(' | '));
      await scroll(page);e=await scan(page);
      if(e.length===0)pass('ZGC Analysis: scroll 0 errors');else fail('ZGC Analysis scroll',e.join(' | '));
