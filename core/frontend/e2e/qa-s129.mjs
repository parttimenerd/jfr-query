/**
 * S129 QA: demo + full interactive suite + CPU Profiling + Heap Allocation
 */
...
(async()=>{
  ...
  console.log('\n══ CPU PROFILING ══');
  {
    ...
    if(ok){
      ...
      e=await scan(page);
      if(e.length===0)pass(`CPU Profiling: ${c} cells, ${s} SVGs, 0 errors`);else fail('CPU Profiling',e.join(' | '));
      await scroll(page);e=await scan(page);
      if(e.length===0)pass('CPU Profiling: scroll 0 errors');else fail('CPU Profiling scroll',e.join(' | '));
  ...
  console.log('\n══ HEAP ALLOCATION ══');
  {
    ...
    if(ok){
      ...
      const s=await page.evaluate(()=>document.querySelectorAll('svg').length);
      e=await scan(page);
      if(e.length===0)pass(`Heap Allocation: ${c} cells, ${s} SVGs, 0 errors`);else fail('Heap Allocation',e.join(' | '));
      await scroll(page);e=await scan(page);
      if(e.length===0)pass('Heap Allocation: scroll 0 errors');else fail('Heap Allocation scroll',e.join(' | '));
