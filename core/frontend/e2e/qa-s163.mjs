/**
 * QA Session S163 — Threading & Contention + Comprehensive Feature Test
 * Rotated templates not recently tested in script-based QA:
 *   Threading & Contention, Comprehensive Feature Test
 * Interactive: variables+popup, LINK_X zoom, command palette,
...
// Zoom any LINK_X chart by dispatching Shift+WheelEvent on its .group wrapper
...
(async () => {
  ...
  check('LINK_X zoom: reset button appeared after Shift+scroll', zoomResult);
  ...
  // ── SECTION 8: Template: Threading & Contention ──────────────────────────
  console.log('\n── Template: Threading & Contention ──');
  try {
    const tcPage = await loadTemplate(browser, 'Threading & Contention');
    const tcCells = await tcPage.locator('.prose, [data-cell]').count();
    const tcCharts = await tcPage.locator('.recharts-wrapper').count();
    const tcErrors = await domScan(tcPage);
    check('Threading & Contention: cells rendered', tcCells > 0, `cells=${tcCells}`);
    check('Threading & Contention: DOM clean', tcErrors.length === 0, tcErrors.join('; '));
    console.log(`  charts=${tcCharts}`);
    await tcPage.close();
  } catch (e) {
    fail++; failures.push('Threading & Contention: ' + e.message.slice(0, 120));
    console.log('  ❌ Threading & Contention FAILED: ' + e.message.slice(0, 120));
  }

  // ── SECTION 9: Template: Comprehensive Feature Test ──────────────────────
  console.log('\n── Template: Comprehensive Feature Test ──');
  try {
    const cfPage = await loadTemplate(browser, 'Comprehensive Feature Test');
    const cfCells = await cfPage.locator('.prose, [data-cell]').count();
    const cfCharts = await cfPage.locator('.recharts-wrapper').count();
    const cfErrors = await domScan(cfPage);
    check('Comprehensive Feature Test: cells rendered', cfCells > 0, `cells=${cfCells}`);
    check('Comprehensive Feature Test: DOM clean', cfErrors.length === 0, cfErrors.join('; '));
    // Comprehensive template should show charts since it has built-in synthetic data
    console.log(`  charts=${cfCharts}`);

    // Also test BRUSH on this template (it uses BRUSH clauses)
    console.log('\n── BRUSH (Comprehensive Feature Test) ──');
    const brushEls = await cfPage.locator('.recharts-brush').count();
    check('BRUSH: recharts-brush elements present in Comprehensive', brushEls > 0, `count=${brushEls}`);
    await cfPage.close();
  } catch (e) {
    fail++; failures.push('Comprehensive Feature Test: ' + e.message.slice(0, 120));
    console.log('  ❌ Comprehensive Feature Test FAILED: ' + e.message.slice(0, 120));
  }
  ...
  // ── Summary ───────────────────────────────────────────────────────────────
  console.log('\n══════════════════════════════════════════');
  console.log('S163 QA: ' + pass + ' passed, ' + fail + ' failed');
