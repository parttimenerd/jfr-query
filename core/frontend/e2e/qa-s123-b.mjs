/**
 * S123 batch-B: Container & Cloud + Exceptions & Errors + Heap Allocation + JVM Internals + Threading & Contention
 */
...
(async()=>{
  ...
  for(const name of['Container & Cloud','Exceptions & Errors','Heap Allocation','JVM Internals','Threading & Contention']){
    console.log(`\nLoading: ${name}`);
  ...
  if(cerrs.length===0)pass('Console: 0 real errors');else{cerrs.forEach(e=>console.log(`  [console error] ${e}`));fail('Console',cerrs.length+' errors');}
