/**
 * S99 template rotation: Memory Leak Detection, Container & Cloud, Exception & Error Analysis
 */
...
async function loadTemplate(page, name) {
  ...
  const idle = await waitForIdle(page, 40000);
  return idle;
}
...
(async () => {
  ...
  const templates = ['Memory Leak Detection', 'Container & Cloud', 'Exceptions & Errors'];
  for (const name of templates) {
    ...
    if (errors.length === 0) {
      pass(`${name}: ${cells} cells, ${svgs} SVGs, 0 DOM errors`);
    } else {
