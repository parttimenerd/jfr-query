/**
 * S140 QA: demo + full interactive suite (BRUSH, tooltips, UI polish)
 *          + Container & Cloud + Memory Leak Detection
 */
...
(async()=>{
  ...
  // Chart tooltip hover
  {
    ...
    if(moved){
      ...
      if(tooltip)pass('Chart tooltip: wrapper present on hover');
      else warn('Chart tooltip: no wrapper visible after mousemove');
  ...
  console.log('\n══ BRUSH ══');
  {
    ...
    if(ok){
      const brushCount=await page.evaluate(()=>
        document.querySelectorAll('.recharts-brush,.recharts-brush-traveller').length
      );
      if(brushCount>0){
        pass(`BRUSH: ${brushCount} brush element(s) found`);
        ...
        if(dragged)pass('BRUSH: drag interaction dispatched');
        else warn('BRUSH: brush-slide not found for drag');
      } else warn('BRUSH: no brush elements in Comprehensive Feature Test');
      e=await scan(page);
    ...
    const issues=await page.evaluate(()=>{
      ...
      document.querySelectorAll('[data-cell-status]').forEach(el=>{
        if(el.getBoundingClientRect().height===0&&el.offsetParent!==null)
          problems.push('zero-height visible cell');
      });
      document.querySelectorAll('.recharts-wrapper').forEach(el=>{
        if(el.scrollWidth>el.clientWidth+8)
          problems.push(`recharts-wrapper overflow (sw=${el.scrollWidth} cw=${el.clientWidth})`);
      });
      return problems.slice(0,5);
    });
    if(issues.length===0)pass('UI polish: no layout issues');
    else warn('UI polish',issues.join(' | '));
  }
  ...
  console.log('\n══ CONTAINER & CLOUD ══');
  {
    ...
    if(ok){
      ...
      e=await scan(page);
      if(e.length===0)pass(`Container & Cloud: ${c} cells, ${s} SVGs, 0 errors`);
      else fail('Container & Cloud',e.join(' | '));
      await scroll(page);e=await scan(page);
      if(e.length===0)pass('Container & Cloud: scroll 0 errors');
      else fail('Container & Cloud scroll',e.join(' | '));
