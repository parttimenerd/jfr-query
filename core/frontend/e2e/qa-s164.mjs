/**
 * QA Session S164 — Recording Overview + CPU Profiling
...
(async () => {
  ...
  check('Demo: charts rendered', demoCharts > 0, `charts=${demoCharts}`);
  ...
  check('SQL autocomplete opens', acFound, `items=${acItems}`);

  // ── SECTION 7: Schema explorer ──────────────────────────────────────────
  console.log('\n── Schema explorer ──');
  const sidebarItems = await demoPage.locator('aside li, [class*="sidebar"] li, [role="tree"] li').count();
  check('Schema explorer: items visible', sidebarItems > 0, `items=${sidebarItems}`);
  ...
  // ── SECTION 9: Template: Recording Overview ──────────────────────────────
  console.log('\n── Template: Recording Overview ──');
  try {
    const ovPage = await loadTemplate(browser, 'Recording Overview');
    const ovCells = await ovPage.locator('.prose, [data-cell]').count();
    const ovCharts = await ovPage.locator('.recharts-wrapper').count();
    const ovErrors = await domScan(ovPage);
    check('Recording Overview: cells rendered', ovCells > 0, `cells=${ovCells}`);
    check('Recording Overview: DOM clean', ovErrors.length === 0, ovErrors.join('; '));
    console.log(`  charts=${ovCharts}`);

    // Also test LINK_X zoom on Recording Overview (it has time-series charts)
    if (ovCharts > 0) {
      const zoomOv = await tryLinkXZoom(ovPage);
      check('Recording Overview: LINK_X zoom works', zoomOv, 'zoom reset appeared');
    }
    await ovPage.close();
  } catch (e) {
    fail++; failures.push('Recording Overview: ' + e.message.slice(0, 120));
    console.log('  ❌ Recording Overview FAILED: ' + e.message.slice(0, 120));
  }

  // ── SECTION 10: Template: CPU Profiling ──────────────────────────────────
  console.log('\n── Template: CPU Profiling ──');
  try {
    const cpuPage = await loadTemplate(browser, 'CPU Profiling');
    const cpuCells = await cpuPage.locator('.prose, [data-cell]').count();
    const cpuCharts = await cpuPage.locator('.recharts-wrapper').count();
    const cpuErrors = await domScan(cpuPage);
    check('CPU Profiling: cells rendered', cpuCells > 0, `cells=${cpuCells}`);
    check('CPU Profiling: DOM clean', cpuErrors.length === 0, cpuErrors.join('; '));
    console.log(`  charts=${cpuCharts}`);
    await cpuPage.close();
  } catch (e) {
    fail++; failures.push('CPU Profiling: ' + e.message.slice(0, 120));
    console.log('  ❌ CPU Profiling FAILED: ' + e.message.slice(0, 120));
  }
  ...
  // ── Summary ───────────────────────────────────────────────────────────────
  console.log('\n══════════════════════════════════════════');
  console.log('S164 QA: ' + pass + ' passed, ' + fail + ' failed');
