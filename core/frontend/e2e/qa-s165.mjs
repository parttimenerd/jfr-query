/**
 * QA Session S165 — All 13 templates + full demo notebook interactive pass
 * Templates: all 13 (Recording Overview, GC Deep Dive, GC Extended, GC Pause Analysis,
 *            CPU Profiling, Heap Allocation, I/O & Latency, JVM Internals,
 *            Memory Leak Detection, Container & Cloud, Exceptions & Errors,
 *            Threading & Contention, ZGC Analysis, Comprehensive Feature Test)
 ...
 * Console errors captured across all pages
 */
...
(async () => {
  ...
  // ── SECTIONS 9+: All templates ──────────────────────────────────────────
  for (const tpl of TEMPLATES) {
    console.log(`\n── Template: ${tpl} ──`);
    try {
      const p = await loadTemplate(browser, tpl);
      const cells = await p.locator('.prose, [data-cell]').count();
      const charts = await p.locator('.recharts-wrapper').count();
      const errors = await domScan(p);
      check(`${tpl}: cells rendered`, cells > 0, `cells=${cells}`);
      check(`${tpl}: DOM clean`, errors.length === 0, errors.join('; '));
      console.log(`  charts=${charts}`);

      // LINK_X zoom test on templates that produce charts
      if (charts > 0) {
        const zoomed = await tryLinkXZoom(p);
        if (zoomed) console.log(`  ✅ LINK_X zoom works`);
      }

      // BRUSH check on Comprehensive Feature Test
      if (tpl === 'Comprehensive Feature Test') {
        const brushEls = await p.locator('.recharts-brush').count();
        check('BRUSH: recharts-brush elements present', brushEls > 0, `count=${brushEls}`);
      }

      await p.close();
    } catch (e) {
      fail++; failures.push(`${tpl}: ${e.message.slice(0, 120)}`);
      console.log(`  ❌ ${tpl} FAILED: ${e.message.slice(0, 120)}`);
    }
  }
  ...
  if (allConsoleMsgs.filter(m => m.type === 'error').length > 0) {
    ...
    allConsoleMsgs.filter(m => m.type === 'error').slice(0, 8)
  ...
  // ── Summary ──────────────────────────────────────────────────────────────
  console.log('\n══════════════════════════════════════════');
  console.log('S165 QA: ' + pass + ' passed, ' + fail + ' failed');
