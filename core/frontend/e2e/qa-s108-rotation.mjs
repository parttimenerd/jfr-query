/**
 * S108 template rotation: Exceptions & Errors + Recording Overview
 */
...
async function scrollScan(page) {
  const errs=new Set();
  for(let s=0;s<=5000;s+=1000){
    await page.evaluate(sc=>{(document.querySelector('main')||document.documentElement).scrollTop=sc;},s);
    await page.waitForTimeout(300);
    const e=await page.evaluate((t)=>Array.from(document.querySelectorAll('*')).filter(el=>{
...
(async()=>{
  ...
  page.on('console',m=>{
    ...
    if(!t.includes('ONNX')&&!t.includes('ort-')&&!t.includes('/api/')&&!t.includes('proxy')
       &&!t.includes('Failed to load resource')&&!t.includes('net::ERR_'))consoleErrs.push(t.slice(0,150));}
  });
  ...
  for(const name of ['Exceptions & Errors','Recording Overview']){
    console.log(`\nLoading: ${name}`);
    ...
    const scanErrs=await scrollScan(page);
    if(scanErrs.length===0)pass(`${name}: ${cells} cells, ${svgs} SVGs, 0 DOM errors`);
    else fail(name,scanErrs.join(' | '));
  }
  if(consoleErrs.length===0)pass('Console: 0 real errors');
  else fail('Console',consoleErrs.join(' | '));
