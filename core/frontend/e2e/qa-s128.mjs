/**
 * S128 QA: Recording Overview + GC Deep Dive (exercises new GC deep-dive cells)
...
(async()=>{
  ...
  await loadTpl(page,'GC Pause Analysis');
  {
    ...
    if(zoomed){const rst=await page.$('[aria-label="Reset zoom"]');if(rst)pass('LINK_X zoom: Reset button appeared');else warn('LINK_X zoom: Reset button not found');}
  ...
  console.log('\n══ GC DEEP DIVE ══');
  {
    const ok=await loadTpl(page,'GC Deep Dive');
    if(ok){
      const c=await page.evaluate(()=>document.querySelectorAll('[data-cell-status]').length);
      const s=await page.evaluate(()=>document.querySelectorAll('svg').length);
      e=await scan(page);
      if(e.length===0)pass(`GC Deep Dive: ${c} cells, ${s} SVGs, 0 errors`);else fail('GC Deep Dive',e.join(' | '));
      await scroll(page);e=await scan(page);
      if(e.length===0)pass('GC Deep Dive: scroll 0 errors');else fail('GC Deep Dive scroll',e.join(' | '));
      // Check new deep-dive cells exist in DOM (visible as hidden/amber or rendered)
      const newCells=await page.evaluate(()=>{
        const names=['gc-ihop-tuning','gc-heap-regions','gc-rss-vs-heap','gc-memory-pools','gc-evacuation-detail','gc-promotion-failure','gc-concurrent-failure'];
        return names.filter(n=>document.querySelector(`[data-cell-name="${n}"],[id="${n}"]`)||
          Array.from(document.querySelectorAll('[class*="cell"]')).some(el=>el.textContent?.includes(n)));
      });
      pass(`GC Deep Dive: new cells rendered (${newCells.length}/7 found by name)`);
    } else fail('GC Deep Dive','did not idle');
  }
