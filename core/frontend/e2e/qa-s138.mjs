/**
 * S138 QA: demo + full interactive suite + JVM Internals + Threading & Contention
 */
...
(async()=>{
  ...
  console.log('\n══ THREADING & CONTENTION ══');
  {
    ...
    if(ok){
      ...
      const s=await page.evaluate(()=>document.querySelectorAll('svg').length);
      e=await scan(page);
      if(e.length===0)pass(`Threading & Contention: ${c} cells, ${s} SVGs, 0 errors`);
      else fail('Threading & Contention',e.join(' | '));
      await scroll(page);e=await scan(page);
      if(e.length===0)pass('Threading & Contention: scroll 0 errors');
      else fail('Threading & Contention scroll',e.join(' | '));
