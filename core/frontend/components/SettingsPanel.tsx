...
const SettingsPanel = forwardRef<any, SettingsPanelProps>(({ metadata, onMetadataChange, isAiFeatureActive, cells = [] }, ref) => {
    ...
    return (<><div className="border border-gray-700/60 rounded-lg overflow-hidden">
        ...
        <div className="px-3 py-2 flex items-center justify-between cursor-pointer hover:bg-gray-700/20" onClick={() => setIsPanelCollapsed(!isPanelCollapsed)} role="button" tabIndex={0} aria-expanded={!isPanelCollapsed} aria-controls="settings-panel-body" onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setIsPanelCollapsed(!isPanelCollapsed); } }}>
