...
const params: PlotParameter[] = [ { name: 'x', type: 'column', required: true, description: 'Column for the X-axis.' }, { name: 'y', type: 'column[]', required: true, description: 'Columns for the primary Y-axis.' }, { name: 'y2', type: 'column[]', description: 'Columns for the second Y-axis.' }, { name: 'color', type: 'column', description: 'Optional column whose distinct values group lines by color (one series per value).' }, { name: 'xDomain', type: 'number[]', description: 'Domain for the X-axis (overrides auto-fit). For time axes, pass numeric ms or quoted ISO timestamps.' }, { name: 'yAxisLabel', type: 'string', description: 'Label for the primary Y-axis.' }, { name: 'y2AxisLabel', type: 'string', description: 'Label for the secondary Y-axis.' }, { name: 'yScale', type: 'string', defaultValue: 'linear', options: ['linear', 'log'], description: 'Scale for the primary Y-axis.' }, { name: 'y2Scale', type: 'string', defaultValue: 'linear', options: ['linear', 'log'], description: 'Scale for the secondary Y-axis.' }, { name: 'yDomain', type: 'number[]', defaultValue: ['auto', 'auto'], description: 'Domain for primary Y-axis.' }, { name: 'y2Domain', type: 'number[]', defaultValue: ['auto', 'auto'], description: 'Domain for secondary Y-axis.' }, { name: 'lineType', type: 'string', defaultValue: 'line', options: ['line', 'dots', 'step', 'stepBefore', 'stepAfter'], description: 'Render as "line", "dots", "step", "stepBefore", or "stepAfter".' }, { name: 'connectNulls', type: 'boolean', defaultValue: false, description: 'Connect lines over nulls.' }, { name: 'xRefLines', type: 'referenceLine[]', description: 'Vertical reference lines.' }, { name: 'yRefLines', type: 'referenceLine[]', description: 'Horizontal reference lines.' }, ];
    ...
    <div style={{ width: '100%', minHeight: 200 }}>
      <ResponsiveContainer width="100%" minHeight={200}>
        <LineChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
          ...
          <YAxis yAxisId="left" stroke="#9ca3af" tick={{fontSize:12}} tickFormatter={yTickFmt ?? yFormatter} scale={mapAxisScale(axisYClause) ?? (effectiveYScale === 'log' ? "log" : "auto")} label={(yLabelFromClause || config.yAxisLabel)?{value:yLabelFromClause || config.yAxisLabel,angle:-90,position:'insideLeft',fill:'#9ca3af',fontSize:12}:undefined} domain={effectiveYScale === 'log' ? (domainY ? [Math.max(0.1, domainY[0]), domainY[1]] : (yDomainFromClause ? [Math.max(0.1, Number(yDomainFromClause[0]) > 0 ? Number(yDomainFromClause[0]) : 0.1), yDomainFromClause[1]] : [Math.max(0.1, Number(config.yDomain?.[0]) > 0 ? Number(config.yDomain?.[0]) : 0.1), config.yDomain?.[1] ?? 'dataMax'])) : (domainY ?? yDomainFromClause ?? config.yDomain) as any} allowDataOverflow/>
          {allY2.length>0 && <YAxis yAxisId="right" orientation="right" stroke="#82ca9d" tick={{fontSize:12}} tickFormatter={y2Formatter} label={config.y2AxisLabel?{value:config.y2AxisLabel,angle:90,position:'insideRight',fill:'#82ca9d',fontSize:12}:undefined} scale={config.y2Scale === 'log' ? "log" : "auto"} domain={config.y2Scale === 'log' ? [Math.max(0.1, Number(config.y2Domain?.[0]) > 0 ? Number(config.y2Domain?.[0]) : 0.1), config.y2Domain?.[1] ?? 'dataMax'] : config.y2Domain as any} allowDataOverflow/>}
          ...
          {allY.map((y,i)=><Line yAxisId="left" key={y} type={config.lineType === 'step' ? 'step' : config.lineType === 'stepBefore' ? 'stepBefore' : config.lineType === 'stepAfter' ? 'stepAfter' : 'monotone'} dataKey={y} stroke={colors[i%colors.length]} connectNulls={config.connectNulls} strokeWidth={config.lineType === 'dots' ? 0 : 1} dot={config.lineType === 'dots'} activeDot={{r: 4}} isAnimationActive={isAnimationActive} animationDuration={animationDuration}/>)}
          {allY2.map((y,i)=><Line yAxisId="right" key={y} type={config.lineType === 'step' ? 'step' : config.lineType === 'stepBefore' ? 'stepBefore' : config.lineType === 'stepAfter' ? 'stepAfter' : 'monotone'} dataKey={y} stroke={colors[(allY.length+i)%colors.length]} connectNulls={config.connectNulls} strokeWidth={config.lineType === 'dots' ? 0 : 1} dot={config.lineType === 'dots'} activeDot={{r: 4}} isAnimationActive={isAnimationActive} animationDuration={animationDuration}/>)}
