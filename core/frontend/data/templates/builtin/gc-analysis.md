...
```plot
LINE_CHART(x: "Time", y: ["IHOP %"]) ON ihop_stats TITLE "G1 IHOP Threshold % Over Time" LINK_X($start, $end) ZOOM AXIS_Y DOMAIN [0, 100] LABEL "%"
```
...
Region count by type over time. Each G1 region is ~1–32 MB depending on heap size.
...
Used and committed bytes per memory pool around each GC cycle.

- **Eden** fills rapidly between GCs and resets to near-zero after a Young GC.
- **Old** grows gradually; high **Used %** signals promotion pressure.
- **Metaspace** growth after classloading peaks = classloader leak.
- **CodeHeap** growth = JIT-compiled code accumulation.
...
```plot
LINE_CHART(x: "Time", y: ["Used %"], color: "Pool") ON pool_data TITLE "Memory Pool Fill % Over Time" LINK_X($start, $end) ZOOM AXIS_Y DOMAIN [0, 100] LABEL "%"
```
...
- **Regions Evacuated** — regions processed per GC.
- **Fill % per Region** — how full each region was on average. Low values (< 50%) = fragmented heap.
- **Promoted MB** — bytes promoted to old gen per GC. Rising = allocation rate exceeds GC throughput.
...
```plot
LINE_CHART(x: "Time", y: ["Fill % per Region"]) ON evac_data TITLE "Evacuation Fill % per Region — low = fragmented" LINK_X($start, $end) ZOOM AXIS_Y DOMAIN [0, 100] LABEL "%"
```
...
Each event means concurrent marking could not complete before the heap was exhausted, forcing a Full GC. Typically caused by:
- Allocation rate exceeding the concurrent mark throughput
- IHOP threshold set too high (mark starts too late)
- Long concurrent mark duration (object graph too large)

Cross-reference the **Adaptive IHOP** cell: if `Last Mark Duration` is rising and failures appear, G1 needs more headroom.
