#!/usr/bin/env python3
"""
Migrate plot_pairs_v22.jsonl → plot_pairs_v23.jsonl by re-extracting signals
with the v11 signal logic:
  - New: `cnt_agg` tag — COUNT() with GROUP BY
    Fires in 58% of PIE examples and only 11% of TREEMAP examples.
  - Impact: PIE/TREEMAP disambiguation greatly improved
    - agg+duo+cnt_agg (no order): 84% PIE, 16% TREEMAP (was 52/48 split)
    - agg+duo without cnt_agg (no order): 67% TREEMAP, 33% PIE (was 48/52)

Run:
    python3 scripts/train/migrate_v22_to_v23.py
"""
...
def extract_input_signals_v11(sql: str, columns: list) -> str:
    sql_up = sql.upper()
    ...
    # v11: cnt_agg — COUNT() with GROUP BY → PIE indicator
...
def remigrate_input(old_input: str) -> str:
    """Re-extract signals using v11 logic, replacing existing hints: line."""
    ...
    signals = extract_input_signals_v11(sql, columns)
...
def main():
    ...
    pairs = [
        (data_dir / 'plot_pairs_v22.jsonl', data_dir / 'plot_pairs_v23.jsonl'),
        (data_dir / 'plot_eval_v22.jsonl',  data_dir / 'plot_eval_v23.jsonl'),
    ]
    print("Migrating v22→v23 (adding `cnt_agg` signal: COUNT() with GROUP BY)...")
    for src, dst in pairs:
        migrate_file(src, dst)
    print("\nDone. Expected impact:")
    print("  PIE examples gain `cnt_agg` tag (58% coverage)")
    print("  TREEMAP gets only 11% `cnt_agg` (vs 58% PIE)")
    print("  agg+duo+cnt_agg (no order) fingerprint → 84% PIE (was ~52%)")
    print("  agg+duo without cnt_agg (no order) → 67% TREEMAP (was ~48%)")
    print("\nTo retrain on v23 data:")
    print("  Update run_training.sh DATA/EVAL to v23, then:")
